...
describe('origin rules', () => {
	...
	it('country: higher income tier scores higher', () => {
		...
		const nl = Math.round(r.position!({ ...DEFAULT_INPUTS, country: 'nl' }) * r.defaultWeight);
		const br = Math.round(r.position!({ ...DEFAULT_INPUTS, country: 'br' }) * r.defaultWeight);
		const af = Math.round(r.position!({ ...DEFAULT_INPUTS, country: 'af' }) * r.defaultWeight);
	...
	it('generational support is monotonic and flagged speculative', () => {
		...
		const s = [0, 1, 2].map((f) => Math.round(r.position!({ ...DEFAULT_INPUTS, familySupport: f as 0 | 1 | 2 }) * r.defaultWeight));
	...
	it('passport strength follows the country henley band', () => {
		...
		expect(Math.round(r.position!({ ...DEFAULT_INPUTS, country: 'us' }) * 6)).toBeGreaterThan(
			Math.round(r.position!({ ...DEFAULT_INPUTS, country: 'in' }) * 6)
		);
	});
});

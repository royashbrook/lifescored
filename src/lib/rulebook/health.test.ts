...
describe('health rules', () => {
	...
	it('life-table: younger scores higher; women slightly higher at same age', () => {
		...
		const young = Math.round(r.position!({ ...DEFAULT_INPUTS, age: 25 }) * r.defaultWeight);
		const old = Math.round(r.position!({ ...DEFAULT_INPUTS, age: 70 }) * r.defaultWeight);
		expect(young).toBeGreaterThan(old);
		const f = Math.round(r.position!({ ...DEFAULT_INPUTS, age: 50, sex: 'f' }) * r.defaultWeight);
		const m = Math.round(r.position!({ ...DEFAULT_INPUTS, age: 50, sex: 'm' }) * r.defaultWeight);
	...
	it('smoking: never > former > current; quit lever applies only to current smokers', () => {
		...
		const s = (smoker: 'never' | 'former' | 'current') => Math.round(r.position!({ ...DEFAULT_INPUTS, smoker }) * r.defaultWeight);
	...
	it('exercise: more minutes never lowers the score; saturates at guideline', () => {
		...
		const s = (m: number) =>
			Math.round(Math.min(hi, Math.max(lo, r.position!({ ...DEFAULT_INPUTS, exerciseMins: m }))) * r.defaultWeight);
	...
	it('sleep: 7–9h band scores best', () => {
		const r = byId('sleep');
		const s = (h: number) => Math.round(r.position!({ ...DEFAULT_INPUTS, sleepHours: h }) * r.defaultWeight);
	...
	it('alcohol: none > moderate > heavy', () => {
		...
		const s = (alcohol: 'none' | 'moderate' | 'heavy') => Math.round(r.position!({ ...DEFAULT_INPUTS, alcohol }) * r.defaultWeight);

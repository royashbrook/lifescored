...
<div class="border-b py-3.5" style:border-color="var(--line)" style:opacity={row.enabled ? 1 : 0.4}>
	<div class="mb-1.5 flex items-baseline justify-between gap-3">
		<div class="flex min-w-0 items-center gap-2">
			<span class="text-[15px]" style:color="var(--ink)">{row.label}</span>
			<Tag kind={row.evidence} />
			{#if !row.enabled}
				<span class="text-[10px]" style:font-family="var(--font-mono)" style:color="var(--ink-dim)">EXCLUDED BY YOU</span>
			{/if}
		</div>
	...
	<div class="mt-1.5 text-[12px] tabular-nums" style:font-family="var(--font-mono)" style:color="var(--ink-dim)">
	...
	<div class="mt-2 text-[13px] leading-normal" style:color="var(--ink-dim)">{row.description}</div>
</div>

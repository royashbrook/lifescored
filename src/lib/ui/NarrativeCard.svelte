...
<div class="mt-5 rounded-lg p-3.5" style:background="var(--panel)" style:border="1px solid var(--line)">
	<div class="mb-1 flex items-center justify-between">
		<div class="text-[12px] tracking-[0.12em]" style:font-family="var(--font-mono)" style:color="var(--ink-dim)">
			IN PLAIN LANGUAGE
	...
	{#if narrative}
		<p class="text-[15px] leading-snug" style:color="var(--ink)">{narrative.text}</p>
		{#if narrative.source === 'local'}
			<p class="mt-1 text-[11px]" style:color="var(--ink-dim)">composed locally from the rulebook — the AI narrator wasn't needed or wasn't available</p>
		{/if}
	{:else}
		<p class="text-[13px] italic" style:color="var(--ink-dim)">A short narrative of what the numbers above are actually saying. Only rounded subtotals ever leave your device.</p>
	{/if}
</div>

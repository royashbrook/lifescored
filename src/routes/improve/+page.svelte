...
{#snippet areaCard(area: ImproveArea)}
	<div class="rounded-lg p-3.5" style:background="var(--panel)" style:border="1px solid var(--line)">
		<div class="mb-1 text-[0.9375rem] font-semibold" style:font-family="var(--font-display)" style:color="var(--ink)">{area.label}</div>
		<div class="mb-2.5 text-[0.8125rem] leading-snug" style:color="var(--ink-dim)">{area.simple}</div>
		<ul class="space-y-1.5 pl-2.5" style:border-left="2px solid var(--sourced)">
			{#each area.free as r (r.url)}
				<li class="text-[0.75rem] leading-snug">
...
<div class="mt-2 space-y-5 text-[0.9375rem] leading-relaxed" style:color="var(--ink)">
	<section>
		...
		<p class="mt-2 text-[0.8125rem]" style:color="var(--ink-dim)">
	...
	{#if startHere.length > 0}
		<section>
			<h2 class="mb-1 text-[1.0625rem] font-semibold" style:font-family="var(--font-display)">Start here — based on your score</h2>
			<p class="mb-2.5 text-[0.8125rem]" style:color="var(--ink-dim)">Your lowest controllable scores right now — the cheapest places to move the number.</p>
	...
	{#each groups as group (group.name)}
		<section>
			<h2 class="mb-2 text-[1.0625rem] font-semibold" style:font-family="var(--font-display)">{group.name}</h2>
			<div class="space-y-2.5">

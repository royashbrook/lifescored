...
<div class="mt-2 space-y-5 text-[13.5px] leading-relaxed" style:color="var(--ink)">
	<section>
		<h2 class="mb-1 text-[16px] font-semibold" style:font-family="var(--font-display)">Use this with an AI</h2>
		<p>
	...
	<section>
		<h2 class="mb-2 text-[16px] font-semibold" style:font-family="var(--font-display)">Easiest: paste this prompt</h2>
		<p class="mb-2.5 text-[12.5px]" style:color="var(--ink-dim)">
		...
		<div class="rounded-lg p-3.5" style:background="var(--panel)" style:border="1px solid var(--line)">
			<p class="text-[12px] leading-relaxed" style:color="var(--ink-dim)">{PROMPT}</p>
			<div class="mt-3"><CopyButton text={PROMPT} label="copy the prompt" /></div>
		</div>
	</section>

	<section>
		<h2 class="mb-2 text-[16px] font-semibold" style:font-family="var(--font-display)">Or connect it directly</h2>
		<div class="space-y-2.5">
			{#each links as l (l.url)}
				<div class="rounded-lg p-3.5" style:background="var(--panel)" style:border="1px solid var(--line)">
					<div class="flex items-center justify-between gap-3">
						<div class="min-w-0">
							<div class="text-[13px] font-semibold" style:color="var(--ink)">{l.label}</div>
							<a href={l.url} class="text-[11px] break-all underline" style:font-family="var(--font-mono)" style:color="var(--start)">{l.url}</a>
						</div>
						<CopyButton text={l.url} label="copy URL" />
					</div>
					<div class="mt-2 text-[12px] leading-snug" style:color="var(--ink-dim)">{l.note}</div>
				</div>
			{/each}
		</div>
		<p class="mt-2.5 text-[12px]" style:color="var(--ink-dim)">
	...
	<section>
		<h2 class="mb-1 text-[16px] font-semibold" style:font-family="var(--font-display)">Make it better</h2>
		<p>
